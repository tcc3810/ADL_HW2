...
def main():
    ...
    def preprocess_function(examples):
        ...
        second_sentences = []
        labels = []
        for i, paragraphs in enumerate(examples[paragraphs_name]):
            tmp = []
            for idx, paragraph in enumerate(paragraphs):
                tmp.append(context_json[paragraph])
                #if examples[relevant_name][i] == paragraph:
                #    labels.append(idx)
                    
            second_sentences.append(tmp)
    ...
    #progress_bar = tqdm(range(args.max_test_steps), disable=not accelerator.is_local_main_process)
    completed_steps = 0
    starting_epoch = 0
    ...
    pred_flatten = np.concatenate(pred , axis=0)
    with open(args.test_file, "r") as f:
        test_datasets = json.load(f)

    for i in range(len(test_datasets)):
        context_id = pred_flatten[i]
        paragraph = context_json[test_datasets[i]["paragraphs"][context_id]]
        test_datasets[i].update({ "context" : paragraph })
    
    with open(args.output_file , "w" , encoding="utf8") as f:
        json.dump(test_datasets , f , ensure_ascii=False)
